"""Consumidor base del broker (patrón de src/_plantilla).

Trío que responde a la caída de una réplica:
  ...
  3. `negative_acknowledge` en error: reentrega en vez de perder el mensaje.
...
Diferencia con la plantilla: el handler recibe `(sobre, mensaje)` — el mensaje
crudo permite usar `mensaje.message_id()` como llave de idempotencia cuando el
productor no llenó `sobre.id` (caso del esquema viejo sin campos de sobre).
"""
import logging
import time
...
class ConsumidorBase:
    def __init__(
        ...
        # manejadores: { "TipoDeMensaje": handler(sobre, mensaje) -> None }
        ...
        while True:
            ...
            try:
                ...
                if handler is None:
                    ...
                    logger.warning("Tipo no manejado '%s'; se ignora", sobre.type)
                    consumidor.acknowledge(mensaje)
                    continue
                handler(sobre, mensaje)           # abre UoW, idempotencia + negocio, commit
